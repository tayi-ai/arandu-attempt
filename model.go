...
// Attempt is the entity this package owns.
//
// It embeds the model, so a row returned by a query carries the connection and
// can be saved again. Build new rows through Attempts: a struct literal has no
...
// Attempts returns the configured model for the attempts table.
//
...
var ErrNotFound = errors.New("attempt: record not found")

// Resource is the list of fields one Attempt is allowed to answer with.
//
...
func NewResource(record Attempt) Resource {
	return newResource(record.ID, record.Name, record.CreatedAt)
}

func resourceFromPointer(record *Attempt) Resource {
	if record == nil {
		return Resource{}
	}
	return newResource(record.ID, record.Name, record.CreatedAt)
...
func NewCollection(records []Attempt, cursor string) Collection {
...
func collectionFromPointers(records []*Attempt, cursor string) Collection {
	resources := make([]Resource, 0, len(records))
	for _, record := range records {
		if record != nil {
