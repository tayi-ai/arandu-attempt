...
`Attempts(db)` configures the table, string primary key and default
`tenant_id` scope. Its terminals return `*Attempt`/`[]*Attempt`; keep those
...
| a CRUD Repository beside the Model | `Attempts(db)`, reached only by `AttemptService` after `security.Authorize` |
...
| a command of its own that copies files into a project | nothing. This package ships no view and publishes no file, so the install is `go get`, three lines in `bootstrap/app.go` and `aru migrate` |
...
2. **Every Service method authorizes before it reaches the Model.**
   ...
   so even constructing `Attempts` in the wrong order fails.
...
4. **Nothing reaches the Model without passing the first two.** The denial
   ...
   `Attempts(nil)` would panic. Every refusal it asserts is therefore proof
...
`Authorize` before its first `Attempts`, every tenant write in the Service
