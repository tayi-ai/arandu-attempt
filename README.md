...
```go
	attemptModule, err := attempt.New(attempt.Config{
...
| `Prefix` | no | where the routes are mounted. Defaults to `/attempt`. |
...
| method | path | name |
| --- | --- | --- |
| `GET` | `/attempt` | `attempt.index` |
| `GET` | `/attempt/{id}` | `attempt.show` |
| `POST` | `/attempt` | `attempt.store` |
...
configured entry point for its table. `AttemptService` owns `*data.DB` and
...
Terminals return `*Attempt` and `[]*Attempt`. Keep those pointers intact:
...
later. The Service calls `security.Authorize` before its first `Attempts(db)`
...
`Attempts(nil)` would panic. The structural twin reads the allowed path and
