...
4. Only then does a `security.Grant` exist, and `service.go` reaches
   `Attempts(s.db)` and a Model terminal with it.
...
`Attempts(s.db)` call is therefore mandatory even though the terminal itself
also takes `g`.
...
`AttemptService` owns `db *data.DB`. Validate first, build the value the Policy
must see, call `security.Authorize`, and only then call `Attempts(s.db)`.
