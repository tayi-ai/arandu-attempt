...
`GET /attempt` answers `{"data": {"items": [...]}}`, and adds
...
`AttemptPolicy` denies every action and has no branch that allows one:
...
There are five actions — `AttemptView`, `AttemptList`, `AttemptCreate`,
`AttemptUpdate`, `AttemptDelete` — and opening one opens exactly one.
