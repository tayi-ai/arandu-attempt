...
```go
	r.Action(stdhttp.MethodDelete, m.cfg.Prefix+"/{id}", m.destroy).Name("attempt.destroy")
```
...
database or called `Attempts` would bypass the only place the Policy is
guaranteed to run. Read `attempt-policy` before writing the Service method.
...
Service results stay as `*Attempt`/`[]*Attempt`, because copying an embedded
