...
// sortableAttempt is the ordering allowlist. A column name taken directly
// from a request would turn ordering into an injection surface.
var sortableAttempt = map[string]string{
	"":           "created_at",
	"name":       "name",
	"created_at": "created_at",
}

// AttemptService holds the rules of this package.
//
...
// NewAttemptService wires the service over the application's database handle.
func NewAttemptService(db *data.DB) *AttemptService {
...
func (s *AttemptService) Create(ctx context.Context, actor security.Subject, in CreateRequest) (*Attempt, error) {
	if errs := in.Validate(); errs.Any() {
		return nil, errs
	}

	proposed := Attempt{Name: in.Name}

	g, err := security.Authorize(ctx, s.policy, actor, AttemptCreate, proposed)
	if err != nil {
		return nil, err
	}
...
func (s *AttemptService) Find(ctx context.Context, actor security.Subject, id string) (*Attempt, error) {
	...
	if _, err := security.Authorize(ctx, s.policy, actor, AttemptView, *record); err != nil {
		return nil, err
	}
	return record, nil
}
...
// run at all.
func (s *AttemptService) List(ctx context.Context, actor security.Subject, q data.Query) ([]*Attempt, error) {
	g, err := security.Authorize(ctx, s.policy, actor, AttemptList, Attempt{})
	if err != nil {
		return nil, err
	}

	column, ok := sortableAttempt[q.Sort]
	if !ok {
		return nil, fmt.Errorf("attempt: sort field not allowed: %q", q.Sort)
	}
