...
2. **Every Service method authorizes before reaching the Model.** The Grant is
   then required by the Model terminal; constructing `Attempts(db)` first is
...
CRUD stays on `Attempts(db)` and its Builder. Add a Repository only for a
...
Model-backed entities stay pointers. Copying `Attempt` also copies an embedded
...
licence, the same as the rest of github.com/tayi-ai/arandu-attempt.
