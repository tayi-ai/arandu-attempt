...
// The actions of Attempt. Constants rather than strings at the call site: a
...
const (
	// AttemptView is reading one record.
	AttemptView security.Action = "attempt.view"
	// AttemptList is paging through the records.
	AttemptList security.Action = "attempt.list"
	// AttemptCreate is adding one.
	AttemptCreate security.Action = "attempt.create"
	// AttemptUpdate is changing one.
	AttemptUpdate security.Action = "attempt.update"
	// AttemptDelete is removing one.
	AttemptDelete security.Action = "attempt.delete"
)

// AttemptPolicy is the only authority over who does what with a Attempt.
//
...
var _ security.Policy[Attempt] = AttemptPolicy{}
...
func (AttemptPolicy) Can(ctx context.Context, s security.Subject, a security.Action, record Attempt) error {
	...
	if record.ID != "" && record.TenantID != s.Tenant {
		return fmt.Errorf("attempt belongs to another tenant")
	}
	...
	//
	//	if a == AttemptView && (s.ID == record.ID || s.HasRole("admin")) {
	//		return nil
	//	}
	//
	...
	//
	//	if a == AttemptView && s.IsGuest() && record.Published {
	//		return nil
	//	}
	// arandu:end custom

	return fmt.Errorf("no rule allows %s on attempt", a)
}
