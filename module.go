// Package attempt is an Arandu module: one entity, one policy that decides
...
func New(cfg Config, db *data.DB, sessions *security.SessionStore) (*Module, error) {
	...
	if db == nil {
		return nil, errors.New("attempt: New needs a database handle: this package owns a table, and there is no in-memory mode that would let it start without one")
	}
	if sessions == nil {
		return nil, errors.New("attempt: New needs a session store: it is where the subject comes from, and a request with no subject cannot be authorized")
	}
	cfg = cfg.withDefaults()
	return &Module{
		cfg:      cfg,
		svc:      NewAttemptService(db),
		sessions: sessions,
	}, nil
}
...
func (m *Module) Name() string { return "attempt" }
...
func (m *Module) Routes(r *fhttp.Router) {
	r.Action(stdhttp.MethodGet, m.cfg.Prefix, m.index).Name("attempt.index")
	r.Action(stdhttp.MethodGet, m.cfg.Prefix+"/{id}", m.show).Name("attempt.show")
...
func (m *Module) Migrations() []foundation.Migration {
	return []foundation.Migration{createAttempts{}}
}
...
var _ migrations.ReversibleMigration = createAttempts{}

// createAttempts is the table this module owns, and the index its listing
// reads by.
type createAttempts struct{ migrations.BaseMigration }
...
func (createAttempts) GetName() string { return "20260823_0001_create_attempts" }
...
func (createAttempts) Up(ctx context.Context, conn migrations.Connection) error {
	return conn.Schema().Create(ctx, "attempts", func(table *schema.Blueprint) {
		...
		table.Index([]string{"tenant_id", "created_at", "id"}, "attempts_tenant_created_idx")
	})
}

// Down drops the table, which takes its index with it.
func (createAttempts) Down(ctx context.Context, conn migrations.Connection) error {
	return conn.Schema().DropIfExists(ctx, "attempts")
}
