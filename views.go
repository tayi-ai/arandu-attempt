...
const (
	...
	viewPrefix = "resources/views/vendor/attempt"
	viewSuffix = ".kyse.go"
)
...
func readArchive() (paths, names []string) {
	...
	if err != nil {
		panic("attempt: reading the embedded views: " + err.Error())
	}
	if len(paths) == 0 {
		panic("attempt: the embedded view directory holds no view, so every name this package renders would be missing and nothing would say so")
	}
	return paths, names
}
...
//
//	resources/views/vendor/attempt/index.kyse.go -> vendor.attempt.index
