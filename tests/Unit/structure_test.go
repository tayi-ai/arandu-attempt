...
func TestTheApplicationIntegrationHasOneCompositionFile(t *testing.T) {
	...
	guides := []string{
		"README.md",
		".agents/skills/attempt-package/SKILL.md",
	}
	...
	for _, guide := range guides {
		...
		text := string(body)
		for _, required := range []string{"bootstrap/app.go", "attempt.New(", "k.Register"} {
...
func TestTheGuidesTeachTheModelFirstBoundary(t *testing.T) {
	...
	guides := map[string][]string{
		"AGENTS.md":                               {"Attempts(db)", "security.Authorize"},
		"README.md":                               {"Attempts(db)", "Model terminal"},
		"CONTRIBUTING.md":                         {"Attempts(db)", "authorizes before reaching the Model"},
		"SECURITY.md":                             {"Model terminal", "data.Tenant(g)"},
		".agents/skills/README.md":                {"CRUD Repository", "configured Model"},
		".agents/skills/attempt-module/SKILL.md":  {"func Attempts", "security.Authorize"},
		".agents/skills/attempt-policy/SKILL.md":  {"security.Authorize", "Attempts("},
		".agents/skills/attempt-release/SKILL.md": {"Model-first", "configured copy"},
	}
	for path, required := range guides {
		...
		for _, stale := range []string{"AttemptRepository", "NewAttemptRepository", "repository.go  data access"} {
