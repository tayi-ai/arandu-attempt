...
func TestEveryServiceMethodAuthorizesBeforeTheModel(t *testing.T) {
	...
	for _, source := range auditedFiles(t) {
		for _, declaration := range source.file.Decls {
			...
			if !ok || function.Body == nil || receiverType(function) != "AttemptService" ||
	...
	if audited == 0 {
		t.Fatal("no exported AttemptService method was found, so this test proved nothing")
	}
}

// firstModelReach is where a Service first constructs the configured Model or
// calls a promoted write terminal. Attempts itself counts: moving only its
...
func firstModelReach(body *ast.BlockStmt) token.Pos {
	...
	ast.Inspect(body, func(node ast.Node) bool {
		...
		if name != "Attempts" && !terminals[name] {
			return true
		}
		if found == token.NoPos || call.Pos() < found {
			found = call.Pos()
