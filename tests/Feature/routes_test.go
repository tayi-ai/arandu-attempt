...
func mount(t *testing.T, cfg attempt.Config) *fhttp.Router {
	t.Helper()

	sessions := security.NewSessionStore([]byte(appKey), time.Hour, false, security.NewMemoryBackend())

	module, err := attempt.New(cfg, data.Wrap(nil, data.DialectSQLite), sessions)
	if err != nil {
...
func TestAVisitorWithNoSessionReachesNothing(t *testing.T) {
	...
	router := mount(t, attempt.Config{Tenant: "acme"})
	...
	for _, request := range []struct {
		...
		{http.MethodGet, attempt.DefaultPrefix, ""},
		{http.MethodGet, attempt.DefaultPrefix + "/record-1", ""},
		{http.MethodPost, attempt.DefaultPrefix, "name=one"},
	} {
...
func TestARejectedInputIsAnsweredBeforeTheDatabase(t *testing.T) {
	...
	// a statement.
	rec := answer(t, router, http.MethodPost, attempt.DefaultPrefix, "name=")
...
func TestTheModuleRegistersItsRoutesUnderItsPrefix(t *testing.T) {
	...
	router := mount(t, attempt.Config{Tenant: "acme", Prefix: "/widgets"})

	got := make([]string, 0, 3)
	for _, route := range router.Routes() {
		if route.Module != "attempt" {
...
func TestNoRouteLandsInTheFrameworkNamespace(t *testing.T) {
	...
	for _, cfg := range []attempt.Config{
		{Tenant: "acme"},
...
func TestNewRefusesAWiringThatCannotWork(t *testing.T) {
	...
	valid := attempt.Config{Tenant: "acme"}

	if _, err := attempt.New(attempt.Config{}, handle, sessions); err == nil {
		t.Error("a configuration with no tenant was accepted")
	}
	if _, err := attempt.New(valid, nil, sessions); err == nil {
		t.Error("a nil database handle was accepted")
	}
	if _, err := attempt.New(valid, handle, nil); err == nil {
		t.Error("a nil session store was accepted")
	}
	if _, err := attempt.New(valid, handle, sessions); err != nil {
...
func TestNewRefusesARoutePrefixThatCannotBeRegistered(t *testing.T) {
	...
	for _, prefix := range []string{"/widgets{", "/widgets/{id}"} {
		if _, err := attempt.New(attempt.Config{Tenant: "acme", Prefix: prefix}, handle, sessions); err == nil {
...
func TestTheModuleDeclaresItsSchema(t *testing.T) {
	...
	sessions := security.NewSessionStore([]byte(appKey), time.Hour, false, security.NewMemoryBackend())
	module, err := attempt.New(attempt.Config{Tenant: "acme"}, data.Wrap(nil, data.DialectSQLite), sessions)
	if err != nil {
		t.Fatalf("building the module: %v", err)
	}
