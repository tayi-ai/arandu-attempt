...
func (c Config) Validate() error {
	if c.Tenant == "" {
		return fmt.Errorf("attempt: Config.Tenant is required: a visitor with no session has to be read as some customer, and it cannot be one the request names")
	}
	...
	if !security.ValidTenant(c.Tenant) {
		return fmt.Errorf("attempt: Config.Tenant is %q, which cannot be a tenant: lowercase letters, digits, - and _, up to 64 characters", c.Tenant)
	}
	if c.Prefix != "" && c.Prefix[0] != '/' {
		return fmt.Errorf("attempt: Config.Prefix is %q and has to start with /", c.Prefix)
	}
	if c.Prefix != "" {
	...
	if c.PageSize < 0 || c.PageSize > MaxPageSize {
		return fmt.Errorf("attempt: Config.PageSize is %d, and has to be between 0 and %d, where 0 means %d", c.PageSize, MaxPageSize, DefaultPageSize)
	}
	return nil
}
...
func validateRoutePrefix(prefix string) (err error) {
	defer func() {
		if recovered := recover(); recovered != nil {
			err = fmt.Errorf("attempt: Config.Prefix %q cannot be registered as a route path", prefix)
		}
	}()
